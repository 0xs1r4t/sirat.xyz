  ...
  <AnimatePresence initial={false}>
    {isOpen && (
      <LazyMotion features={loadFeatures}>
        {/* Mobile max-height reserves 9.5rem at the top + bottom-2's 0.5rem
            = 10rem. The 9.5rem matches ThemeSwitcherButton's 3-button
            stack (8.75rem tall, measured) plus 0.75rem — the same gap the
            theme buttons leave *between each other* (gap-1's 0.25rem +
            each button's own mt-2 stacking on top of it, not just gap-1
            alone), so the panel's top edge reads as "one more
            button-width away" rather than an arbitrary number. Desktop
            keeps the original top-16/5rem pair, unrelated. */}
        <m.div
