// One instance per published post. Ported from web-terrain flower.vert
// (Y-locked billboard, per-instance wind variation). The LOD density cull
// is removed — every post's flower must always be visible — and
// instanceHover drives the hover-scale animation.
...
void main() {
    ...
    float flowerVariation = fract(sin(instanceRand * 6.2831) * 43758.5453);
    ...
    float scale = 1.0 + instanceHover * 0.18;
